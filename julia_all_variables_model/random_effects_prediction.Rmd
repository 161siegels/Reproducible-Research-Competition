...
### Get improvement over baseline accuracy
...
baselines = data.frame(Player=0, baseline=0)
for(i in unique(test$Player_ID)) {
  shots = test %>%
    filter(Player_ID == i)
  makes = sum(shots$SHOT_RESULT2)
  misses = nrow(shots) - makes
  baselines = rbind(baselines, data.frame(Player=i, baseline=(getMax(makes, misses)) / nrow(shots)))
}
baselines = baselines[-1,]
```

# Get Improvement from Baseline Per Player
```{r}
player_accuracies$Improvement = 0
for(i in player_accuracies$Player) {
  player_accuracies$Improvement[i] = player_accuracies$Accuracy[i] - baselines$baseline[i]
}
head(player_accuracies)
```

# Get Top Relevant Baseline Improvements
```{r}
player_accuracies_with_name = player_accuracies %>%
  filter(Player %in% player_shot_counts$Player_ID) %>%
  top_n(10, wt = Improvement) %>%
  inner_join(test, by = c("Player" = "Player_ID")) %>%
  dplyr::select(Player, player_name, Accuracy, Improvement) %>%
  unique() %>%
  arrange(desc(Improvement))
player_accuracies_with_name
mean(player_accuracies$Improvement)
```
```{r}
ggplot(data=player_accuracies_with_name) +
  geom_point(mapping = aes(x=player_name,y=Improvement))
```
